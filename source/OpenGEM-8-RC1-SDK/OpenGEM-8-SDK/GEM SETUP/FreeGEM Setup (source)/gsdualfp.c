...
/* CHECK FOR DRIVER DISK */
	for (ii = 0; ii < NUM_REL_DSKS + num_drvr_paks; ii++)
		if (!strcmp(disk_label[ii], gl_dlabel))
			break;
	if (!get_vol2( vol_label[ii], gl_dsrcpath))
	{
		SCRN_11.choice[0].todo_func = finish_startup ;
		into_drive = gl_src_path[0];
		insert_disk_labeled = gl_dlabel ;
		beeper() ;
		return ;
	}
	
/* CHECK FOR STARTUP DISK */
	if (!get_vol2(SU_VOL, DEFAULT_DRV))
	{
		SCRN_11.choice[0].todo_func = finish_startup ;
	...
	init_copy_echo();
	if ((!strcmp( gl_dlabel, disk_label[1])) || /* GEM SCREEN DISK #1 */
	    (!strcmp( gl_dlabel, disk_label[2])))   /* GEM SCREEN DISK #2 */
		dev_install(TRUE);
	else
		dev_install(FALSE);
...
/* CHECK SYSTEM DISK */
	if (!get_vol2( SYS_VOL, gl_src_path))
		{
...
/* CHECK THAT NEW STARTUP DISK IS A REL OR DRVR OR DT DISK */
	ok_disk = TRUE;
	vol = is_vol(SYS_VOL, DEFAULT_DRV);
	if (vol == 0)
		{
		for (ii = 0; ii < 11; ii++)
			temp[ii] = gl_dta[ii+8];
...
/* CHECK FOR DRIVER DISK */
	...
	if (!get_vol2( vol_label[ii], gl_src_path))
		{
...
/* CHECK FOR DESKTOP DISK */
	...
	init_copy_echo();
	file_copy(gl_src_path, gemsys, mdsys, FALSE);
	hide_drivers();
	dev_install(FALSE);
	dev_change();
...
/* GET SCREEN DISK (one that's needed or any) */
		...
		if (insert_disk_labeled == NULLPTR )
	        ...
	        gl_dlabel   = insert_disk_labeled ;
		gl_dsrcpath = insert_disk_srcpath ;
		into_drive = gl_src_path[0] ;
		SCRN_11.choice[0].todo_func = start2_desktop ;
		}
	else
