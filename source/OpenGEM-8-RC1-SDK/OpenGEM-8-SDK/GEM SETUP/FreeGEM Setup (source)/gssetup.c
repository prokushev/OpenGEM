...
EXTERN VOID	new_device(VOID) ;	/* gsnewdev.c */
EXTERN VOID	ch_device(VOID) ;	/* gsnewdev.c */
...
BOOLEAN	found_1st;
   ...
   if (prev_screen != 11)
    ...
    b_stuff( misc_choices, sizeof( misc_choices ), NULL ) ;
    found_1st = FALSE;
    ...
    for (ptr1 = W11_PROMPT; ptr1 < end; ptr1++)
		...
		if ( *ptr1 == PIPE ) 
			...
			*ptr1 = PIPE ;
			...
			misc_choices[0][strlen(misc_choices[0])] = into_drive;
...
setup_16()
{
	setup_78(W16_PROMPT,gl_src_path[0]);
...
WORD	ii ;
	...
	if ((devices[ii].type != SCAN_TYPE) && 
	    ...
	    strcat( W17_PROMPT, five_spaces ) ;
	    strcat( W17_PROMPT, five_spaces ) ;
	    strcat( W17_PROMPT, choices[ 8 + devices[ ii ].ioport ] ) ;
	    strcat( W17_PROMPT, CRLF ) ;
	    }
        if ( devices[ ii ].type == SCAN_TYPE)
		    show_scan_stuff( ii );		
...
setup_18()
{
	...
	if (( devices[ ii ].type != SCAN_TYPE ) && 
	    ...
	    strcat(W18_CHOICES+jj*kk,choices[8+devices[ii].ioport]);
