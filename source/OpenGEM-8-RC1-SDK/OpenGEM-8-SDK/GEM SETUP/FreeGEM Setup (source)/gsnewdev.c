...
EXTERN WORD	_min(WORD a, WORD b) ;			/* gsutil.c */
...
new_port() 
{
    ...
    if ( devices[ cur_dev ].type == PRNT_TYPE )
    	devices[ cur_dev ].ioport = picked ;
    else
	{
    	devices[ cur_dev ].ioport = picked + 3 ;	/* jump past LPTs */
	ioport = picked + 3;
	}
    ...
    for (ii = 0; ii < num_devs ; ii++)
	    if  (ii != cur_dev) 
		    if (devices[ii].ioport != NO_PORT)
			    if (devices[ii].ioport == ioport)
				    no_match = FALSE;
...
do_lbcopy( strt_ptr, AUX_FILE, FNAME_LENGTH, devices[ cur_dev ].aux_file ) ;
    ...
    if (strt_ptr >= txt_gemtop) 
         strcpy(devices[cur_dev].src_path, gl_src_path);
    else strcpy(devices[cur_dev].src_path, gem_src_path);
    found = FALSE;
...
BYTE	tmp[ NAME_LENGTH ] ;
	...
	devices[ cur_dev ].ioport = NO_PORT ;
...
FBYTE	*ptr ;
{
    ...
    devices[ num_devs ].ioport  = (WORD)( *ptr++ ) ;
...
/*----------------------------------------------------------------------*/
/* Add another text file to our driver database */
BOOLEAN append_txt_buf(const char *src)
{
	LONG    size, count ;
	WORD	handle ;
	...
	handle = open_file( src, gemsettxt, 0 ) ;
...
/*----------------------------------------------------------------------*/
VOID get_new_txt(VOID)
{
	/* TODO This is where we should switch to a new source path. 
	 * Maybe even prompt the user for it rather than assuming A:\ */

	if (!(get_vol2(DPK_VOL, gl_src_path)))
		scrn_node[16-1].choice[1].next = 16;
	else if (!append_txt_buf(gl_src_path))
		/* WRONG TXT FILE */;
} /*get_new_txt*/
